"""Experiment 1: Context-length comparison
seq200_original vs seq500_original vs seq1000_original
...
OUT_DIR = os.path.join(EVAL_ROOT, 'context_experiment')
os.makedirs(OUT_DIR, exist_ok=True)
STEP = 990000
SPLIT = 'val'

MODELS = {
    'seq200_original':  {'label': 'seq200 (original)',  'color': '#2196F3', 'ls': '-'},
    'seq500_original':  {'label': 'seq500 (original)',  'color': '#FF9800', 'ls': '--'},
    'seq1000_original': {'label': 'seq1000 (original)', 'color': '#4CAF50', 'ls': '-.'},
}
...
# All models share the same labels
labels = np.load(os.path.join(EVAL_ROOT, 'seq200_original', SPLIT, 'labels.npy'))
...
for group_name, (start, end) in GROUPS.items():
    ...
    for name, info in MODELS.items():
        ...
        roc_auc = auc(fpr, tpr)
        ax.plot(fpr, tpr, color=info['color'], linestyle=info['ls'], linewidth=2,
                label=f"{info['label']} (AUC={roc_auc:.4f})")
    ax.plot([0, 1], [0, 1], 'k:', alpha=0.4, label='Random')
    ax.set_xlabel('False Positive Rate', fontsize=12)
    ax.set_ylabel('True Positive Rate', fontsize=12)
    ax.set_title(f'ROC — {group_name} (Validation)  |  Context-length comparison', fontsize=14, fontweight='bold')
...
MODEL_LOGS = {
    'seq200_original':  [f'{SHARED}/train_seq200.log'],
    'seq500_original':  [f'{SHARED}/train_seq500.log'],
    'seq1000_original': [
...
ax.set_title('Training Loss — Context-length comparison', fontsize=14, fontweight='bold')
